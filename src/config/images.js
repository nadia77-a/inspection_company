export default {
  img1: require(`../assets/images/img1.jpg`),
  img2: require(`../assets/images/img2.jpg`),
  logo: require(`../assets/images/logo.png`), logo1: require(`../assets/images/logo1.png`),

  certif1: require(`../assets/images/certif1.jpg`),
  certif2: require(`../assets/images/certif2.jpg`),
  certif3: require(`../assets/images/certif3.jpg`),
  certif4: require(`../assets/images/certif4.jpg`),
  certif5: require(`../assets/images/certif5.jpg`),


  d1: require(`../assets/images/d1.jpg`),
  d2: require(`../assets/images/d2.jpg`),
  d3: require(`../assets/images/d3.jpg`),
  d4: require(`../assets/images/d4.jpg`),
  d5: require(`../assets/images/d5.jpg`),

  i1: require(`../assets/images/i1.jpg`),
  i2: require(`../assets/images/i2.jpg`),
  i3: require(`../assets/images/i3.jpg`),
  i4: require(`../assets/images/i4.jpg`),
  i5: require(`../assets/images/i5.jpg`),
  i6: require(`../assets/images/i6.jpg`),
  i7: require(`../assets/images/i7.jpg`),

  w1: require(`../assets/images/w1.png`),
  w2: require(`../assets/images/w2.jpg`),
  w3: require(`../assets/images/w3.jpg`),
  w4: require(`../assets/images/w4.jpg`),
  w5: require(`../assets/images/w5.jpg`),
  w6: require(`../assets/images/w6.jpg`),

  
  q1: require(`../assets/images/q1.jpg`),
  q2: require(`../assets/images/q2.jpg`),
  q3: require(`../assets/images/q3.jpg`),
  q4: require(`../assets/images/q4.jpg`),
  q5: require(`../assets/images/q5.jpg`),
  q6: require(`../assets/images/q6.jpeg`),
  q7: require(`../assets/images/q7.jpg`),

  o1: require(`../assets/images/o1.jpg`),
  o2: require(`../assets/images/o2.jpg`),
  o3: require(`../assets/images/o3.jpg`),
  o4: require(`../assets/images/o4.jpg`),
  o5: require(`../assets/images/o5.jpg`),

};
